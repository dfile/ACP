...
hashset_t hashset_create()
{
    ...
    set->nbits = 3;
    set->capacity = (int)(1 << set->nbits);
    set->mask = set->capacity - 1;
    set->items = calloc(set->capacity, sizeof(int));
    if (set->items == NULL) {
        hashset_destroy(set);
        return NULL;
    }
...
static int hashset_add_member(hashset_t set, int item)
{
    int value = item;
    size_t ii;
...
static void maybe_rehash(hashset_t set)
{
    ...
    if ((float)set->nitems >= (size_t)((double)set->capacity * 0.85)) {
        ...
        for (ii = 0; ii < old_capacity; ii++) {
            hashset_add_member(set, (int)old_items[ii]);
        }
        free(old_items);
    }
}

int hashset_add(hashset_t set, int item)
{
...
int hashset_remove(hashset_t set, int item)
{
    int value = item;
...
int hashset_is_member(hashset_t set, int item)
{
    int value = item;
    size_t ii = set->mask & (prime_1 * value);
