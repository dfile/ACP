...
struct LinkedListArray* transformOrbit(number quad[4], struct LinkedListArray* orbit) {
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        {
        */
            //llaAppend(orbit, nodeArrayInitWithArray(family[0]));
            nodeA *node = nodeArrayInit();
            memcpy(node->val, family, sizeof(family));
            llaAppend(solutions, node);
        //}
    //}
...
struct LinkedListArray* genealogy(number seed[4]) {
    ...
    if (ancestors.len != 0)
    {
        ...
        number len = ancestors.len;
        for (i = 0; i < len; i++)
        {
            printf("Ancestors len: %d", (int)ancestors.len);
            llaPrint(&ancestors);
            temp = llaPop(&ancestors);
            nodeArrayPrint(temp);
